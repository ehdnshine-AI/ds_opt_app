...
$OnPremDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoRoot = Split-Path -Parent $OnPremDir
$EnvFile = if ($env:ENV_FILE) { $env:ENV_FILE } else { Join-Path $OnPremDir ".env" }
$PostgresComposeFile = if ($env:POSTGRES_COMPOSE_FILE) { $env:POSTGRES_COMPOSE_FILE } else { Join-Path $OnPremDir "postgres.compose.yml" }
...
$RunDir = if ($env:RUN_DIR) { $env:RUN_DIR } else { Join-Path $OnPremDir ".run" }
$LogDir = if ($env:LOG_DIR) { $env:LOG_DIR } else { Join-Path $OnPremDir "logs" }
...
$SolverRequestTimeout = if ($env:SOLVER_REQUEST_TIMEOUT) { $env:SOLVER_REQUEST_TIMEOUT } else { "120" }
$HighsThreads = if ($env:HIGHS_THREADS) { $env:HIGHS_THREADS } else { "4" }
$OmpNumThreads = if ($env:OMP_NUM_THREADS) { $env:OMP_NUM_THREADS } else { $HighsThreads }
...
Usage:
  ...
  .\on-prem\run.windows.ps1 deploy
  .\on-prem\run.windows.ps1 start-solvers
  .\on-prem\run.windows.ps1 start-api
  .\on-prem\run.windows.ps1 start-all
  .\on-prem\run.windows.ps1 restart-all
  .\on-prem\run.windows.ps1 stop-solvers
  .\on-prem\run.windows.ps1 stop-api
  .\on-prem\run.windows.ps1 stop-all
  .\on-prem\run.windows.ps1 status
  .\on-prem\run.windows.ps1 postgres-up
  .\on-prem\run.windows.ps1 postgres-down
  .\on-prem\run.windows.ps1 postgres-logs
  .\on-prem\run.windows.ps1 postgres-ps

Defaults:
  env file: $EnvFile
  postgres compose: $PostgresComposeFile
"@
}
...
function Require-Runtime {
    if (-not (Test-Path -LiteralPath $PythonBin)) {
        ...
        Write-Host "Run: .\on-prem\run.windows.ps1 install"
        exit 1
    }
}
...
function Install-Deps {
    ...
    & $PythonBin -m pip install -r (Join-Path $RepoRoot "requirements.txt")
...
function Invoke-PostgresCompose {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Args
    )

    $composeArgs = Get-PostgresComposeArgs
    & docker compose @composeArgs @Args
}

function Get-PostgresComposeArgs {
    $composeArgs = @("-f", $PostgresComposeFile)
...
function Test-DockerEngineAvailable {
    return (Test-Path "\\.\pipe\dockerDesktopLinuxEngine") -or (Test-Path "\\.\pipe\docker_engine")
}
...
function Show-StatusAll {
    ...
    if ((Get-Command docker -ErrorAction SilentlyContinue) -and (Test-Path -LiteralPath $PostgresComposeFile)) {
        Write-Host "postgres:"
        if (-not (Test-DockerEngineAvailable)) {
            Write-Host "[warn] docker compose status unavailable"
        } else {
            $composeArgs = Get-PostgresComposeArgs
            $output = & docker compose @composeArgs ps 2>&1
...
function Deploy-All {
    Write-Host "[info] Deploying current Python source from: $RepoRoot"
    Install-Deps
    Stop-Api
    Stop-Solvers
    Start-Solvers
...
switch ($Command) {
    ...
    "postgres-up" { Invoke-PostgresCompose up -d }
    "postgres-down" { Invoke-PostgresCompose down }
    "postgres-logs" { Invoke-PostgresCompose logs -f postgres }
    "postgres-ps" { Invoke-PostgresCompose ps }
